...
// Description : Quiz on implementing kd tree
//===============================================================================//
...
private:
    ...
    void recursiveInsert(Node **node, const std::vector<float> point,
                         const int id, const int coord)
    {
        ...
        recursiveInsert((goLeft ? &(*node)->left : &(*node)->right), point, id,
                        ((coord + 1) % 3));
    }

    bool withinBox(const std::vector<float> target, const Node *node,
                   const float tolerance)
    {
        ...
        const bool withinXbounds{node->point.at(X) >= leftX &&
                                 node->point.at(X) <= rightX};

        const float lowerY{target.at(Y) - tolerance};
        const float upperY{target.at(Y) + tolerance};

        const bool withinYbounds{node->point.at(Y) >= lowerY &&
                                 node->point.at(Y) <= upperY};

        const float lowerZ{target.at(Z) - tolerance};
        const float upperZ{target.at(Z) + tolerance};

        const bool withinZbounds{node->point.at(Z) >= lowerZ &&
                                 node->point.at(Z) <= upperZ};

        return (withinXbounds && withinYbounds && withinZbounds);
    }

    float withinRange(const std::vector<float> p1, const std::vector<float> p2,
                      const float tolerance)
    {
        ...
        const float distance{std::sqrt((xComponent * xComponent) +
    ...
    void recursiveSearch(Node *node, const std::vector<float> &target,
                         std::vector<int> &ids, const float tolerance,
                         const int coord)
    {
        ...
        if (withinBox(target, node, tolerance) &&
            withinRange(node->point, target, tolerance))
