...
// Description : PCL lib Functions for processing point clouds
//===============================================================================//
...
// // #include "process-point-clouds/processPointClouds.h"
...
//     typename pcl::PointCloud<PointT>::Ptr filteredVoxelCloud{
...
//     typename pcl::PointCloud<PointT>::Ptr filteredCroppedCloud{
//         new pcl::PointCloud<PointT>()};
...
//     {
//         std::cout << "Failed to segment points." << std::endl;
//     }
...
//         // I have ABSOLUTELY no clue why the map was sometimes only inserting 2
//         // indices
//         // I shouldn't have to do this, but a seg fault was occurring without this
//         // check
//         if (inliers.size() == 2)
//         {
//             inliers.insert(rand() % cloud->points.size());
...
//         const float i{
//             static_cast<float>(((y2 - y1) * (z3 - z1)) - ((z2 - z1) * (y3 - y1)))};
//         const float j{
//             static_cast<float>(((z2 - z1) * (x3 - x1)) - ((x2 - x1) * (z3 - z1)))};
//         const float k{
//             static_cast<float>(((x2 - x1) * (y3 - y1)) - ((y2 - y1) * (x3 - x1)))};
...
//         {
//             const float plane{std::fabs((A * cloud->points.at(index).x) +
...
//     std::vector<bool> &processed, const int index, const float tolerance,
//     KdTree *tree)
// {
...
//     const std::vector<int> nearbyPoints{
//         tree->search(points.at(index), tolerance)};
...
//     const std::vector<std::vector<int>> listOfIndices{
//         euclideanCluster(points, kdTree, clusterTolerance)};
...
//     std::cout << " milliseconds and found " << clusters.size() << " clusters"
//               << std::endl;
