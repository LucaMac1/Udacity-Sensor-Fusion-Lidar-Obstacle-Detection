...
// Description : Simple 3d highway enviroment using PCL for exploring
//               self-driving car sensors
//===============================================================================//

#include <process_point_clouds/process_point_clouds.h>
...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr &viewer)
{
    ...
    renderPointCloud(viewer, segmentCloud.first, "Obstacle Cloud",
                     Color(1, 0, 0));
               ...
               const pcl::PointCloud<pcl::PointXYZI>::Ptr &inputCloud)
{
    ...
    const pcl::PointCloud<pcl::PointXYZI>::Ptr filteredCloud{
        pointProcessor->FilterCloud(inputCloud, 0.1f,
                                    Eigen::Vector4f(-(X / 2), -Y, -Z, 1),
                                    Eigen::Vector4f(X, Y, Z, 1))};

    renderPointCloud(viewer, filteredCloud, "filteredCloud");

    const std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr,
                    pcl::PointCloud<pcl::PointXYZI>::Ptr>
        segmentCloud{//     pointProcessor->SegmentPlane(filteredCloud, 100, 0.2)
    ...
    const std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters{
        pointProcessor->Clustering(segmentCloud.first, 0.35, 30, 1775)};
    ...
    const std::vector<Color> colors{Color(1, 0, 1), Color(0, 1, 1),
                                    Color(1, 1, 0)};
